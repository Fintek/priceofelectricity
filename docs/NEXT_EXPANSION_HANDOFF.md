...
- **SEO hygiene / canonical cleanup (Prompt 1, 2026-04-13):** Repo audit found **no** current internal links, sitemap rows, or `contentRegistry` city nodes emitting legacy city URL shapes; city surfaces consistently use `/electricity-cost/{state}/{city}`. **Verification:** `scripts/legacyCitySitemapGuards.ts` plus matching checks in `indexing-readiness.ts` and `readiness-audit.ts` block legacy `/{state}/city/...` and two-segment `/{state}/{city}` aliases from appearing in segmented sitemaps. **Intentionally unchanged:** legacy redirect routes remain for inbound/bookmark support only.
- **Structural SEO — navigation and discovery (2026-04-14):** Primary header and mobile nav now link to `/electricity-cost-comparison` and `/energy-comparison` (replacing legacy `/compare` as the main “compare” entry). `/energy-comparison` includes a visible “Comparison guides” block linking to `/energy-comparison/states`, `/usage`, and `/appliances`. `/electricity-cost-comparison` includes a crawlable alphabetical index (`#all-comparisons`) of all pairs from `pairs.json`. `/electricity-cost/[state]` lists every active city for that state (removed the previous eight-city cap) with user-facing city section copy. **Verification:** `indexing-readiness` and `readiness-audit` assert shell links, sub-hub links, comparison index anchor, and Texas city hrefs. **Deferred:** broad anchor-text sweep, utility cross-linking, footer architecture redesign, cross-family contextual linking beyond these surfaces.
- **CTR / intent-framing audit (Prompt 2, 2026-04-13):** Identified and fixed internal-jargon and generic-framing issues across 8 canonical template families. **Templates targeted:** `/electricity-cost/[state]`, `/electricity-cost/[state]/[city]`, `/electricity-cost-comparison/[pair]`, `/energy-comparison` (hub + 3 children), `/average-electricity-bill/[state]`. **Problems found:** meta descriptions and intro paragraphs contained engineering terms ("configured reference rate", "deterministic electricity-cost context", "canonical pair system", "curated slice", "authority scope", "rollout-gated", "discovery directory"), generic page-about-itself framing ("This page summarizes…"), and missing answer-first content on comparison pair pages. **Changes:** rewrote meta descriptions for user intent; replaced intro paragraphs with answer-first, data-specific copy; removed exposed internal authority language; added jargon-regression guard to `scripts/seo-check.ts` (blocks "canonical", "curated slice", "rollout-gated", "allowlist", "pilot", "discovery directory" in meta descriptions); expanded SEO check coverage to 3 canonical template paths. **Verification:** `verify:vercel` fully green (65/0 indexing, 79/0 readiness, 11/0 SEO, 27/0 smoke, 1429/0 integrity, payload audit passed). **Next track:** Prompt 3 — measure CTR/impression changes in GSC after indexing stabilizes; then evaluate deeper answer-first framing or snippet optimization if data supports it.
