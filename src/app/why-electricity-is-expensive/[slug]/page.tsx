...
export async function generateMetadata({
  ...
  if (!statePage) {
    return buildMetadata({
      ...
      canonicalPath: `/why-electricity-is-expensive/${slug}`,
    });
  }
  ...
  const description =
    avgRate != null
      ? `Why electricity is expensive in ${stateName}. Average rate ${avgRate.toFixed(2)}¢/kWh. Factors that can influence electricity prices.`
      : `Why electricity is expensive in ${stateName}. Factors that can influence electricity prices.`;
  return buildMetadata({
    title: `Why Electricity Is Expensive in ${stateName} | PriceOfElectricity.com`,
    description,
    canonicalPath: `/why-electricity-is-expensive/${slug}`,
  });
}

export default async function WhyElectricityIsExpensiveStatePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const breadcrumbJsonLd = buildBreadcrumbListJsonLd([
    ...
    { name: `Why Electricity Is Expensive in ${stateName}`, url: `/why-electricity-is-expensive/${slug}` },
  ]);
    ...
    <>
      ...
      <main className="container">
        <nav aria-label="Breadcrumb" className="muted" style={{ marginBottom: 16, fontSize: 14 }}>
          <Link href="/">Home</Link>
          {" · "}
          <Link href="/why-electricity-is-expensive">Why Electricity Is Expensive</Link>
          {" · "}
          <span aria-current="page">{stateName}</span>
        </nav>

        <h1 style={{ fontSize: 32, marginBottom: 24 }}>
          Why Electricity Is Expensive in {stateName}
        </h1>

        {/* A) Intro */}
        <section style={{ marginBottom: 24 }}>
          <p style={{ marginTop: 0, marginBottom: 16, maxWidth: "65ch", fontSize: 16, lineHeight: 1.6 }}>
            {stateName}&apos;s electricity price may be influenced by regional market structure, generation context, infrastructure, and fuel exposure. This page provides context—not state-specific causal explanations—using the state&apos;s current electricity price data.
          </p>
        </section>
          ...
          <section style={{ marginBottom: 24 }}>
            ...
            <p style={{ marginTop: 0, marginBottom: 12, maxWidth: "65ch", fontSize: 16, lineHeight: 1.6 }}>
              ...
              {nationalAvg != null && comparison && (
                <> That is {Math.abs(comparison.differencePercent).toFixed(1)}% {comparison.differencePercent > 0 ? "above" : "below"} the national average of {nationalAvg.toFixed(2)} ¢/kWh.</>
              )}
            </p>
            <p style={{ margin: 0 }}>
        ...
        <section style={{ marginBottom: 24 }}>
          ...
          <ul style={{ margin: 0, paddingLeft: 20, lineHeight: 1.8 }}>
            <li>Regional market structure and regulation</li>
            <li>Generation mix and fuel exposure</li>
            <li>Grid infrastructure and transmission costs</li>
            <li>Supply-demand balance and capacity</li>
          </ul>
        </section>
        ...
        <section style={{ marginBottom: 32 }}>
          ...
          <ul style={{ margin: 0, paddingLeft: 20, lineHeight: 2.2 }}>
            ...
            <li><Link href="/why-electricity-is-expensive">Why electricity is expensive (national)</Link></li>
