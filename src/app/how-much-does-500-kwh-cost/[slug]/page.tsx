...
export async function generateMetadata({
  ...
  const description =
    avgRate != null && cost != null
      ? `How much does 500 kWh cost in ${stateName}? At ${avgRate.toFixed(2)}¢/kWh, approximately $${cost.toFixed(2)}. State electricity cost estimate.`
      : `Electricity cost for 500 kWh in ${stateName}. State electricity price data.`;
  return buildMetadata({
    title: `How Much Does 500 kWh Cost in ${stateName}? | PriceOfElectricity.com`,
    description,
    canonicalPath: `${BASE_PATH}/${slug}`,
  });
}
    ...
    <>
      ...
      <main className="container">
        <nav aria-label="Breadcrumb" className="muted" style={{ marginBottom: 16, fontSize: 14 }}>
          <Link href="/">Home</Link>
          {" · "}
          <Link href={BASE_PATH}>How Much Does 500 kWh Cost?</Link>
          {" · "}
          <span aria-current="page">{stateName}</span>
        </nav>

        <h1 style={{ fontSize: 32, marginBottom: 12 }}>
          How Much Does 500 kWh Cost in {stateName}?
        </h1>
        ...
        {avgRate != null && (
          ...
          >
            ...
            >
              <div className="muted" style={{ fontSize: 12, marginBottom: 4 }}>Estimated cost (500 kWh)</div>
          ...
          <section style={{ marginBottom: 32 }}>
            ...
            >
              <p style={{ margin: 0, fontSize: 16, lineHeight: 1.6 }}>
                At 500 kWh, electricity in {stateName} costs approximately{" "}
        ...
        <section style={{ marginBottom: 32 }}>
          ...
          <ul style={{ margin: 0, paddingLeft: 20, lineHeight: 1.8 }}>
            ...
            <li><Link href={BASE_PATH}>How much does 500 kWh cost? (all states)</Link></li>
          </ul>
        </section>
